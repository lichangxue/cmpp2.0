 ...
 * 增强、简化正则功能
 */
ifeng.RegExp = ifeng.createClass();
ifeng.RegExp.prototype = {
	_reg:null,
 ...
 */
ifeng.DateFormat = ifeng.createClass();
ifeng.DateFormat.prototype = {
	_pattern: null,
	/**
	 * @param string 所要显示日期的样式
	 */
	initialize: function(pattern) {
		this._pattern = pattern?pattern:ifeng.DateFormat.MEDIUM;
	},
	/**
	...
	format: function(date) {
		...
		for(var f in ifeng.DateFormat.FormatCodes) {
			formattedDateStr = formattedDateStr.replace(eval('/' + f +'/g'), eval('dateObj.' + ifeng.DateFormat.FormatCodes[f]));
		}
 ...
 * 
 */
ifeng.Clock = ifeng.createClass();
ifeng.Clock.prototype = {
	...
	initialize:function(){
		ifeng.EventDispatcher.bless(this);
	},
	...
	start:function(){
		...
		this._state = 'started';
		this._hInterval = setInterval(ifeng.delegate(this,this._beat) ,this.getIntervalTime());
		this._fire('start');
	},
...
ifeng.loader = {
	...
	loadJs:function(url){
		...
		var jsData = ifeng.ajax.sync.get(url);
		if(!jsData){
			throw new Error('fail to load js:'+url);
		}
		ifeng.loader._includeJs(jsData);
		this._jsLoaded(url);
		return true;
	},
	...
	loadPackage:function(packageName){
		if(ifeng.loader.isPackageLoaded(packageName)){
			return false;
		};
		var url =ifeng.loader.getJsRoot()+'/'+ packageName.replace(/\./g,'/')+'.js';
		return ifeng.loader.loadJs(url);
	},
	/**
	 * 加载样式表
	 * @param url String
	 ...
	 * @return boolean
	 */
	loadIfengCss:function(file){
		return ifeng.loader.loadCss(ifeng.loader.getJsRoot()+'/ifeng/'+file);
	},
	/**
	 * 加载jquery js用的css样式
	 * @param file String
	...
	loadJQueryCss:function(file){
		return ifeng.loader.loadCss(ifeng.loader.getJsRoot()+'/jquery/'+file);
	},
	/**
	 * @param string packageName
	 * @return boolean
	...
	loadJQueryJs:function(file){
		var url = ifeng.loader.getJsRoot()+'/jquery/'+file;
		return ifeng.loader.loadJs(url);
	},
	getJsRoot:function(){
		return ifeng.loader._jsRoot;
	},
	/**
	 * 设置ifengJS的根路径 ifeng.js所在目录的上一级，不以/结束
	 */
	setJsRoot:function(url){
		ifeng.loader._jsRoot = url;
	},
	/**
	...
	_isJsLoaded:function(url){
		var pool = ifeng.loader._loadedJs;
	...
	_isCssLoaded:function(url){
		var pool = ifeng.loader._loadedCss;
		for(var i=0;i<pool.length;i++){
			if(pool[i]==url){
				return true;
			}
