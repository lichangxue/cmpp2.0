...
Ext.Ajax = new Ext.data.Connection({
    ...
    defaultHeaders:{ajax:true},//roney added to fix server-side authorization problem;
...
Ext.extend(Ext.util.MixedCollection, Ext.util.Observable, {
    ...
    findIndexBy : function(fn, scope, start){
		...
		/* cds modify fixed bug at 2012/5/15
...
Ext.extend(Ext.dd.DragSource, Ext.dd.DDProxy, {
    ...
    /* cds modify at 2011-3-7
    beforeDragDrop : function(target, e, id){
        return true;
    },
	*/ 
	/*cds fixed at 2011-3-7 鎷栨嫿鍒板祵濂楃洰鏍囩殑鍐掓场澶勭悊*/
	beforeDragDrop : function(target, e, id){
    ...
    labelSeparator : ':',
        ...
        if(!this.fieldTpl){
                ...
                '<div class="x-form-item {5}" tabIndex="-1">',
					...
					//'<div style="width: 20px;height: 20px;  background:#0080C0; float: right;cursor:move; padding-top: 2px;"></div>',//cds add
                    '</div><div class="{6}"></div>',
                '</div>'
            );
...
Ext.Button = Ext.extend(Ext.Component, {
    ...
    hasVisibleMenu : function(){
        ...
        if(!this.disabled){
            if(this.enableToggle!=false && (this.allowDepress !== false || !this.pressed)){
                this.toggle();
            }
...
Ext.extend(Ext.Editor, Ext.Component, {
    ...
    startEdit : function(el, value){
        ...
        //roney:var v = value !== undefined ? value : this.boundEl.dom.innerHTML;
    ...
    onBlur : function(){
        if(this.allowBlur !== true && this.editing){
	        this.completeEdit();
	        //#1000:roney to fix content overpassing it's cell(grid is added from gridpanel.startEditing);
	        (this.grid&&this.grid.ensureIntegrity());
	        //#1000 end;
        }
    },
...
Ext.form.Field = Ext.extend(Ext.BoxComponent,  {
    ...
    getName: function(){
        ...
        if(!this.el){
			...
			/*cds modify 濡傛灉鏈寚瀹歯ame锛屽氨缂虹渷涓嶅埗瀹歯ame锛屽師鐗堢己鐪乶ame鍜孖d涓€鏍?
            if(!cfg.name){
...
Ext.form.TextField = Ext.extend(Ext.form.Field,  {
	...
	////////cds add///////
    /* Roney removed to commonUI_cfg.js;
	setFieldLabel : function(text) {
		Ext.fly(this.el.dom.parentNode.previousSibling).update(text+':');
	},
	
	setHideLabel:function(isHide){
		Ext.fly(this.el.dom.parentNode.previousSibling).setVisible(!isHide);
	}
	*/
	/*
...
Ext.form.DateField = Ext.extend(Ext.form.TriggerField,  {
    ...
    getValue : function(){
		...
		return this.formatDate(this.parseDate(Ext.form.DateField.superclass.getValue.call(this)) || "");//cds add
    },
	...
	onTriggerClick : function(){
		...
		this.menu.picker.setValue(this.parseDate(this.getValue()) || new Date());//cds 
...
Ext.form.ComboBox = Ext.extend(Ext.form.TriggerField, {
    ...
    initComponent : function(){
		...
		///////cds add//////
		if(!this.store){
			if(this.initialConfig.dataSource){
				this.mode = 'local';
				var data =this.initialConfig.dataSource;
				...
				this.valueField = 'value';
				this.displayField = 'text';
			}else if(this.initialConfig.url){
				this.mode = 'local';
				this.store = new Ext.data.SimpleStore({　
				　　fields:['value','text'],　
					autoLoad :true,
				...
				this.valueField = 'value';//值
				this.displayField='text';//显示文本
			}
		}
		///////cds add end/////
        ...
        //roney:force hidden be used.
		this.hiddenName=this.name;
		this.hiddenId="h_"+this.hiddenName;//avoid two same ids of el and hidden;
    },
	...
	////////cds add///////
	setFieldLabel : function(text) {
		Ext.fly(this.el.dom.parentNode.parentNode.previousSibling).update(text);
	},
	
	setHideLabel:function(isHide){
		Ext.fly(this.el.dom.parentNode.parentNode.previousSibling).setVisible(!isHide);
	}
...
Ext.extend(Ext.form.BasicForm, Ext.util.Observable, {
    ...
    submit : function(options){
        ...
        this.el.fireEvent("submit");//roney:store value of all customed controls;
...
Ext.FormPanel = Ext.extend(Ext.Panel, {
	...
	initFields : function(){
        ...
        if(this.items) this.items.each(fn);
    },
...
Ext.form.HtmlEditor = Ext.extend(Ext.form.Field, {
    ...
    createToolbar : function(editor){
        ...
        if(this.enableFont && !Ext.isSafariNotChrome){
                ...
                '-', {
                    ...
                    menu : new Ext.menu.ColorMenu({
                        ...
                        selectHandler: function(cp, color){
                            this.execCmd('forecolor', Ext.isSafariNotChrome || Ext.isIE ? '#'+color : color);
                            this.deferFocus();
                        },
                        scope: this,
                        clickEvent:'mousedown'
                    ...
                    menu : new Ext.menu.ColorMenu({
                        ...
                        selectHandler: function(cp, color){
                            if(Ext.isGecko){
                                ...
                                this.execCmd(Ext.isOpera ? 'hilitecolor' : 'backcolor', Ext.isSafariNotChrome || Ext.isIE ? '#'+color : color);
                                this.deferFocus();
                            }
                        },
                        scope:this,
    ...
    onRender : function(ct, position){
        ...
        this.tb.items.each(function(item){
		   ...
		   if(item.itemId != 'sourceedit' && item.sourceEditStateEnable!=2){//cds add
                item.disable();
            }
        });
    ...
    toggleSourceEdit : function(sourceEditMode){
        ...
        if(this.sourceEditMode){
            this.tb.items.each(function(item){
				//if(item.itemId != 'sourceedit'){
				if(item.itemId != 'sourceedit' && item.sourceEditStateEnable!=true){//cds add
					item.disable();
                }
            });
            this.syncValue();
    ...
    initEditor : function(){
        ...
        var ss = this.el.getStyles('font-size', 'font-family', 'background-image', 'background-repeat','background-attachment');//cds modify,允许自定义背景
        ss['background-attachment'] = ss['background-attachment']||'fixed'; 
        //dbody.bgProperties = 'fixed'; 
		dbody.bgProperties = ss['background-attachment'] || 'fixed'; 
        ...
        if(Ext.isIE || Ext.isSafariNotChrome || Ext.isOpera || Ext.isWebKit){
    ...
    adjustFont: function(btn){
        ...
        if(Ext.isSafari3 || Ext.isAir){
            ...
            v = Math.max(1, v+adjust) + (Ext.isSafariNotChrome ? 'px' : 0);
        }
    ...
    updateToolbar: function(){
        ...
        if(!Ext.isSafariNotChrome && this.enableLists){
...
Ext.form.Label = Ext.extend(Ext.BoxComponent, {
    onRender : function(ct, position){
        if(!this.el){
			...
			if(this.id)this.el.id = this.id;//cds add
            if(this.forId){
...
Ext.grid.GridPanel = Ext.extend(Ext.Panel, {
    ...
    ensureIntegrity:function(){
    	this.view.scroller.dom.scrollLeft=0;	
    },
    //#1000 end;
...
Ext.extend(Ext.grid.GridView, Ext.util.Observable, {
    ...
    scrollToTop : function(){
      ...
      var mb = this.scroller.dom;
        ...
        for(var i = 0, len = ns.length; i < len; i++){
            ns[i].style.width = tw;
			if(ns[i].firstChild){//cds modify bug at 2012/5/16
				ns[i].firstChild.style.width = tw;
				if(ns[i].firstChild.rows) ns[i].firstChild.rows[0].childNodes[col].style.width = w;
			}
        }
        ...
        for(var i = 0, len = ns.length; i < len; i++){
			...
			if(ns[i].firstChild){//cds modify bug at 2012/5/16
				ns[i].firstChild.style.width = tw;
				if(ns[i].firstChild.rows) ns[i].firstChild.rows[0].childNodes[col].style.display = display;
			}
        }
