...
Ext.ux.RowNumberer = function(config){
    Ext.apply(this, config);
    if(this.rowspan){
        this.renderer = this.renderer.createDelegate(this);
    }
...
Ext.ux.RowNumberer.prototype = {
    ...
    renderer : function(v, p, record, rowIndex){
        ...
        return '<a title="双击可输入排序号进行排序">' + (rowIndex+1) + "</a>";
    },
...
Ext.ux.TuiJianWei.V3 = Ext.extend(Ext.form.Field, {
	...
	hideStickButton :false,
	hidePublishDateTime :false,
	hidePublisher:false,
	hideOrderBy:true,//固定、随机、置顶 radio按钮
	enableDragDrop:true,//可拖拽排序
	addBtnTxt: "手工添加",
	mainPanelCls: "NewsPublisher_Panel",
	maxReserveRecordCount:200,//最大保留记录数,-1表示不限制
	enableEditFields:'title',//可编辑的字段
	afterEditHandler:function(data){
		//console.log("afterEditHandler:" + data);
	},
	metaData: null,
	store:null,
	searchResult:null,
	...
	initComponent: function () {
		...
		if(this.initialConfig.afterEditHandler && typeof this.initialConfig.afterEditHandler=="string"){
			try{
				this.initialConfig.afterEditHandler = eval('(' + this.initialConfig.afterEditHandler + ")");
			}catch(e){
				console.error('afterEditHandler应该是一个function');
			}
		}
		Ext.applyDeep(this,this.initialConfig);
		if(typeof this.fields ==="string") this.fields = Ext.decode(this.fields);
		this.fields.push("sys_publisher","sys_publishDateTime","id","sys_orderBy");
	...
	getValue: function () {
		...
		this.el.dom.value =this.metaData.root.length==0?"":Ext.encode(this.metaData);
	...
	setValue: function (v) {
		...
		if(this.store){
			...
			if(rootData.length>this.viewSize){
				...
				this.mainPanel.bbar.setDisplayed(true);
			}else{
				this.mainPanel.bbar.setDisplayed(false);
			}
			data["root"] = rootData;
	...
	initTitleRuler:function(){
		if(this.rulerNum1 && parseFloat(this.rulerNum1)>0){
			this.rulerNums.push(parseFloat(this.rulerNum1));
		}
		if(this.rulerNum2 && parseFloat(this.rulerNum2)>0){
			this.rulerNums.push(parseFloat(this.rulerNum2));
		}
		if(this.rulerNum3 && parseFloat(this.rulerNum3)>0){
			this.rulerNums.push(parseFloat(this.rulerNum3));
		}
		if(this.rulerNum4 && parseFloat(this.rulerNum4)>0){
			this.rulerNums.push(parseFloat(this.rulerNum4));
		}
	},
	...
	initMainPanel:function(ct, position){
		...
		var searchBtnHandler = function(button,index){
			...
			win.show(null,function(win){
				...
				win.el.fadeIn({
					...
					scope:this,
					callback:function(){
						if((win.height>Ext.getBody().getHeight())||(win.width>Ext.getBody().getWidth())){
							win.maximize();
						}
		...
		var columns = [];
		if(!this.hideRowNumberer) columns.push(new Ext.ux.RowNumberer({width:28}));
		...
		for(var i=0;i<this.columnsCfg.length;i++){
			...
			if(col.isView!=false){
				...
				if(("," + this.enableEditFields + ",").indexOf("," + col.field + ",")!=-1){//字段可编辑
		...
		if(!this.hideOrderBy) {
			var radioColumn = new Ext.grid.RadioColumn({
				...
				width:100,
				align:'center',
				dataSource:[["default","固"],["random","随"],["top","置顶"]],
				dataIndex:"sys_orderBy",
				scope:this,
				callback:function(grid,record,row,field,newValue){
					this.metaData.root[row][field] = newValue;
				}
			});
			columns.push(radioColumn);
			plugins.push(radioColumn);
		}
		...
		var mainPanel=new Ext.grid.EditorGridPanel({
			...
			autoHeight: this.height=="auto" || !this.height,
			...
			onEditComplete : function(ed, value, startValue){
				...
				if(this.forceValidation === true || String(value) !== String(startValue)){
					...
					if(this.fireEvent("validateedit", e) !== false && !e.cancel && String(value) !== String(startValue)){
						if(field!="numberer") r.set(field, e.value);
						delete e.cancel;
				...
				if(field=="numberer" && (parseInt(value)+"")!="NaN"){//排序数字栏修改了顺序号
					//重新排序
					this.context.moveTo(this.context.store,ed.row,value-1);
					this.stopEditing();	
				}else{
					var d=this.context.metaData.root[index];
					d[field]=value;
					if(typeof this.context.afterEditHandler == "function"){
						this.context.afterEditHandler.call(this,{
							record:r,
							field:field,
							originalValue:startValue,
							value:value
	...
	moveTo:function(store,oldRow,newRow,curDom){
		if (oldRow >=0 && newRow>=0) {
			if(newRow>=store.data.length) newRow=store.data.length-1;
			var record = store.getAt(oldRow);

			this.metaData.root.splice(oldRow,1);
			this.metaData.root.splice(newRow,0,record.data);

			store.remove(record);
			store.insert(newRow, record);
			this.mainPanel.selModel.selectRow(newRow,true);
			//让选中行在屏幕可见
			var rowEl = Ext.fly(this.mainPanel.body.select(".x-grid3-row").elements[newRow]);
			rowEl && rowEl.scrollIntoView(rowEl.parent().parent(),false);
