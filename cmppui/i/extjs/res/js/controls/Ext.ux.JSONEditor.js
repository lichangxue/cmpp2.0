...
Ext.extend(Ext.ux.JSONEditor, Ext.form.Field, {
	/****/
	resRoot:'../res/',
	rowAddAndDeleteEnable:true,//行是否可操作
	...
	getValue:function(){
		return this.hiddenInput.value || this.value;
	},
	setValue:function(v){
		
		if(typeof v =="string"){
			Ext.ux.JSONEditor.superclass.setValue.call(this,v);
			try{
				v = Ext.decode(v);
			}catch(ex){
				//alert("数据格式不正确");
