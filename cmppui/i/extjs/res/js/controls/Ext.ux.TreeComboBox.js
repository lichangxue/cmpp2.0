...
Ext.ux.TreeComboBox=function(config){
    ...
    _tree.on('click',function(node){
		var text = _combobox._getFullTextByNode(node);//_combobox._getTextByValue(node,_combobox.valueField);
...
Ext.extend(Ext.ux.TreeComboBox ,Ext.form.ComboBox, {
	...
	//根据节点获取节点完整的层级text
	_getFullTextByNode:function(node){
		var text='';
		//递归计算出完整text路径
		node.bubble(function(){
			text = this.text !=='' && this.text !=='根节点' ? (this.text + '>' +  text) : text ;
		});
