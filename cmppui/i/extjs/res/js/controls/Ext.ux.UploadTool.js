...
Ext.extend(Ext.ux.UploadTool, Ext.form.TextField, {
	...
	beforeNoteText2:'例如：http://p1.ifengimg.com/a/2010/0601/ 以/结束',
	beforeNoteText3:'例如：http://p1.ifengimg.com/a/2010/0601/a.jpg 文件名要有扩展名',
	width:300,
    ...
    initComponent: function () {
		...
		this.emptyText = this.uploadMode==2?'例如：http://p1.ifengimg.com/a/2010/0601/ 以/结束':'例如：http://p1.ifengimg.com/a/2010/0601/a.jpg 文件名要有扩展名'
	...
	onRender: function (ct, position) {
		...
		*/
		...
		if(this.uploadMode==1 || this.uploadMode==2){
			var autoFileNameCt = elParent.createChild({
				tag:"span",
				cls:"UploadTool-ct-check",
				html:'<input type="checkbox" id="'+ this.id + '_chk' +'"/><label for="' + this.id + '_chk' +'">自动生成文件名</label>'
			});
			this.autoFileNameChk = autoFileNameCt.child("input[type=checkbox]");
			this.autoFileNameChk.dom.checked = this.autoFileName;
			this.autoFileNameChk.dom.onchange = function(t){
				return function(){
					t.autoFileName = this.checked;
				}
			}(this);
		}
	...
	fileUploadHandler:function(e,obj,opts){
		...
		for (var i = 0, file; file = arrFiles[i]; i++) {
			if (file.type.indexOf("image") == 0) {
				...
				var fileJson = {file:file};	
				this.doUpolad(fileJson);
			}
		}
	},
	doUpolad:function(fileJson){
		//开始上传
		var params = this.setParams(fileJson.file,this.reuploadRow);
		if(!params) return;
		if(fileJson.img){//图片
			var img = fileJson.img;
			var pos = params.fileName.lastIndexOf(".");
			var imginfo = "size" + img.k + "_w" + img.width + "_h" + img.height;
			var filename;
			if(this.autoFileName){
				filename = params.fileName.substring(0,pos) + imginfo + params.fileName.substring(pos);
			}else{
				filename = params.fileName.substring(0,pos) + params.fileName.substring(pos);
			}
		...
		if(reuploadRow!==-1){//重新上传
			...
			if(mode==1 || mode==2){
				...
				if(this.autoFileName){
					...
					params.fileName = hex_md5(new Date().valueOf()+file.name+parseInt(Math.random()*1000)).substring(0, 15).toLowerCase() +file_ext
				}else{
