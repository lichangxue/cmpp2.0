 ...
 Ext.override(Ext.form.HtmlEditor, {
	...
	insertAtCursor : function(text){
		...
		if (!selection.isCollapsed) { 
			if(selection.focusNode.nodeName=="#text"){//fix chrome
		...
		if(url && url != 'http:/'+'/') {
			...
			if (sel) {
				if(sel.focusNode.nodeName=="#text"){
					...
					this.insertAtCursor('<a target="_blank" href="'+url+'">'+sel.focusNode.innerHTML+'</a>');
				}
...
Ext.form.HtmlEditor2 = Ext.extend(Ext.form.HtmlEditor,{
	resRoot:'../res/',
	uploadUrl:'../upload!file.jhtml?needCompress=true',
	...
	imageWidthScale:4,//宽高比例 裁图时用
	imageHeightScale:3,//宽高比例
	targetWidth:400,
	targetHeight:300,
	width:700,
	height:600,
	...
	initComponent: function () {
		...
		this.enableMyLinks && plugins.push(new Ext.form.HtmlEditor.Link(),new Ext.form.HtmlEditor.CancelLink());
			...
			var readClipboardDataCallback=function(edtior){
				return function(clipboardData){
					...
					for(var i=textlines.length-1;i>=0;i--){
						if(i==0){//插入到本段内部
							if(node.data)
								node.data = node.data.substr(0,focusOffset) + textlines[0] + node.data.substr(focusOffset);
							else
								node.innerText = textlines[0];
						}else{
							var newNode = document.createElement("p");
							newNode.innerText = textlines[i];
							var targetNode = node.nodeType==1?node:node.parentNode;
							insertAfter(newNode,targetNode);
							/*
							Ext.fly(node.parentNode).insertSibling({
								tag:"p",
								html:textlines[i]
							},"after");
							*/
	...
	/*uploadAndCutImage:function(obj,targetWidth,targetHeight){
		if(this.uploadAndCutImageWindow){
			...
			var formPanel = new Ext.FormPanel({
				...
				autoScroll : true, 
				itemCls:'subItemStyle',
				border : false, 
				items:[{
					fieldLabel:'宽高比例',
					xtype:"label",
					html:'<input type="number" value="'+ this.imageWidthScale +'" />:<input type="number" value="'+ this.imageHeightScale +'" />'
				},{
					fieldLabel:'目标宽度',
					xtype:'numberfield',
					name:'targetWidth',
...
IFengTextFormatter.prototype = {
	...
	format:function(content){
		...
		content = content.replace(/<a\s+[^>]*>(.*?)<\/a>/ig,function(all, front, tag){
			if(all.indexOf('.ifeng.com')!=-1){
				return all.replace(/<a/i,'(((anchor_begin)))').replace(/<\/a>/i,'(((anchor_end)))');
			}else{
				return front;
			}
		});
	...
	_filter:function(ps){
		...
		for(var i=0; i<ps.length; i++){
			...
			if(!currentIsImage){
				...
				/*/p = p.replace( new RegExp("<.*?>(.*?)</.*?>",'g'),function(all, front, tag){
					if(all.indexOf('.ifeng.com')!=-1){
						return all;
					}else{
						return front;
			...
			postTag = "</p>";
			if(currentIsImage){
				//考虑图片上一层有链接的情况 cds add
				var preContent = out[out.length-1];
				if(preContent.indexOf('(((anchor_begin)))')!=-1){
					preTag = '<p class="detailPic">' + preContent.replace('<p>','').replace('</p>','');
					postTag = '(((anchor_end)))</p>';
					out.pop();
				}else{
...
IFengSpecialTagProcessor.prototype = {
	...
	_revertIfengLink:function(c){
		c= c.replace(/\({3}anchor_begin\){3}/g,'<a').replace(/\({3}anchor_end\){3}/g,'</a>');
		return c;
	},
	...
	revertContent:function(content){
		...
		return this._revertBUIs(this._revertRNs(this._revertComments(this._revertIfengLink(content))));
	}
};
//================强力排版 结束========================
...
Ext.form.HtmlEditor.UploadAndCutImage = Ext.extend(Ext.util.Observable, {
	...
	// private
    init: function(cmp){
		this.targetWidth = cmp.targetWidth;
		this.targetHeight = cmp.targetHeight;
		this.imageWidthScale = cmp.imageWidthScale;
		this.imageHeightScale = cmp.imageHeightScale;
        this.cmp = cmp;
        this.cmp.on({
            'render': this.onRender,
            scope: this
	...
	uploadAndCutImage:function(obj,targetWidth,targetHeight){
		...
		if(this.uploadAndCutImageWindow){
			...
			var widthScaleId = that.id +'_widthScale';
			var heightScaleId = that.id +'_heightScale';
			var fixedScaleId = that.id +'_fixedScale';
			this.uploadAndCutImageWindow = new Ext.Window({ 
				...
				items : [{
					...
					labelWidth : 55, 
					layout:'xform',
					id: that.id + '_insert-cutimage',
					frame : true, 
					bodyStyle : 'padding:5px 5px 0', 
					autoScroll : true, 
					itemCls:'subItemStyle',
					border : false, 
					id:that.id + "_formPanel",
					items:[{
						fieldLabel:'宽高比例',
						xtype:"panel",
						border:false,
						...
						html:'宽高比例：<input type="text" id="'+ widthScaleId + '" style="width:30px;text-align:right;" value="'+ this.imageWidthScale +'" />:<input type="text" id="'+ heightScaleId +'"  style="width:30px" value="'+ this.imageHeightScale +'" />  <input id="'+ fixedScaleId +'" type="checkbox" checked="checked"/><label for="'+fixedScaleId+'">  固定宽高比例</label>'
					},{
						fieldLabel:'目标宽度',
						xtype:'numberfield',
						name:'targetWidth',
						...
						openCutterWindow:function(obj){
							...
							values.targetWidth && values.targetHeight && win.setTargetSize(parseInt(values.targetWidth),parseInt(values.targetHeight));
							values.imageUrl && win.setOriginalImageUrl(values.imageUrl);
							win.show();
						}
					}]
				}],
				...
				buttons : [{ 
					...
					handler:function(obj){
						var win = obj.ownerCt;
						var frm = win.findById(this.id + '_formPanel').getForm();
						if (frm.isValid()) {
			...
			this.formPanel = this.uploadAndCutImageWindow.findById(this.cmp.id + "_formPanel").getForm(); 
			this.txtTargetWidth = this.formPanel.findField("targetWidth");
			this.txtTargetHeight = this.formPanel.findField("targetHeight");

			this.txtWidthScale = Ext.get(widthScaleId);
			this.txtHeightScale = Ext.get(heightScaleId);
			this.chkFixedScale = Ext.get(fixedScaleId);
			//obj && win.alignTo(obj.el,"br",[-300,3]);
		
			function onchange(textbox,e){
				if(this.chkFixedScale.dom.checked){
					this.txtTargetHeight.setValue(Math.floor(parseInt(this.txtTargetWidth.getValue())*parseInt(this.txtHeightScale.getValue())/parseInt(this.txtWidthScale.getValue())));
				}
			}

			this.txtWidthScale.on("keyup",onchange,this);
			this.txtHeightScale.on("keyup",onchange,this);
			this.chkFixedScale.on("change",onchange,this);
			
			this.txtTargetWidth.el.on("keyup",onchange,this);
			this.txtTargetHeight.el.on("keyup",function(textbox,e){
				if(this.chkFixedScale.dom.checked){
					this.txtTargetWidth.setValue(Math.floor(parseInt(this.txtTargetHeight.getValue())*parseInt(this.txtWidthScale.getValue())/parseInt(this.txtHeightScale.getValue())));
				}
			},this);
		}
            ...
            handler: function(){
                if (!this.tableWindow){
                    this.tableWindow = new Ext.Window({
                        ...
                        closeAction: 'hide',
                        items: [{
                            itemId: this.cmp.id + '_insert-table',
                            xtype: 'form',
                            border: false,
                            plain: true,
                        ...
                        buttons: [{
                            text: this.langInsert,
                            handler: function(){
                                var frm = this.tableWindow.getComponent(this.cmp.id + '_insert-table').getForm();
                                if (frm.isValid()) {
            ...
            handler: function(){
                ...
                if (!this.farWindow){
                    ...
                    this.farWindow = new Ext.Window({
                        ...
                        items: [{
							columnWidth: .68,
                            itemId: this.cmp.id + '_findandreplace',
                            xtype: 'form',
                            border: false,
                            plain: true,
                            bodyStyle: 'padding: 10px;',
                ...
                this.farWindow.show();
                this.farForm = this.farWindow.getComponent(this.cmp.id + '_findandreplace').getForm();
            },
            scope: this,
            tooltip: {
                title: this.langTitle
...
Ext.form.HtmlEditor.Link = Ext.extend(Ext.util.Observable, {
    ...
    // private
	cmd: 'link',
    linkTargetOptions: [ ['_blank', '_blank'],['_self', '_self'], ['_parent', '_parent'], ['_top', '_top']],
    init: function(cmp){
    ...
    onRender: function(){
        var cmp = this.cmp;
        var btn = this.cmp.getToolbar().addButton({
			iconCls: 'x-edit-addlink',
			cls : "x-btn-icon",
			icon : this.cmp.resRoot + "img/runTime/link_add.png", 
            handler: function(){
                if (!this.linkWindow) {
                    this.linkWindow = new Ext.Window({
                        ...
                        items: [{
                            xtype: 'form',
                            itemId: this.cmp.id + '_insert-link',
                            border: false,
                            plain: true,
                            bodyStyle: 'padding: 10px;',
                            labelWidth: 40,
                        ...
                        buttons: [{
                            text: this.langInsert,
                            handler: function(){
                                var frm = this.linkWindow.getComponent( this.cmp.id + '_insert-link').getForm();
                                if (frm.isValid()) {
                        ...
                        listeners: {
                            show: {
                                fn: function(){
                                    ...
                                    var frm = this.linkWindow.getComponent(this.cmp.id + '_insert-link').getForm();
                                    var txt = frm.findField('text');
									txt.setValue(sel.textContent)
									txt.setDisabled(sel.hasHTML);
                                    var u = frm.findField('url');
									u.reset();
									u.focus(true, 50);
                                },
                                scope: this,
...
/**
 * @author chengds
 * @class Ext.form.HtmlEditor.AddLink
 * @extends Ext.util.Observable
 * <p>添加超链接</p>
 */
Ext.form.HtmlEditor.AddLink = Ext.extend(Ext.util.Observable, {
        //language text
        langAddLink: '添加超链接',
    // private
    cmd: 'addlink',
    ...
    onRender: function(){
        this.btn = this.cmp.getToolbar().addButton({
			...
			scope: this,
            tooltip: {
                title: this.langAddLink
            },
            overflowText: this.langAddLink
		});
		
	},
	createLink:function(){
		this.cmp.createLink();//ext默认的
...
/**
 * @author chengds
 * @class Ext.form.HtmlEditor.CancelLink
 * @extends Ext.util.Observable
 * <p>取消超链接</p>
 */
Ext.form.HtmlEditor.CancelLink = Ext.extend(Ext.util.Observable, {
        //language text
        langCancelLink: '取消超链接',
    // private
    cmd: 'cancellink',
    // private
    init: function(cmp){
    ...
    onRender: function(){
        this.btn = this.cmp.getToolbar().addButton({
            iconCls: 'x-edit-fullscreen',
			cls : "x-btn-icon",
			icon : this.cmp.resRoot + "img/runTime/cancelLink.png", 
            handler: this.cancelLink,
			scope: this,
            tooltip: {
                title: this.langCancelLink
            },
            overflowText: this.langCancelLink
		});
		
	},
	//取消链接
	cancelLink:function(fullScreen){
		var that = this.cmp;
		var sel = false;
		if (window.getSelection) {
			sel = that.win.getSelection();
		} else if (document.getSelection) {
			sel = that.doc.getSelection();
		}
		if (sel) {
			...
			if(sel.focusNode.nodeName=="#text"){
				...
				if(node.tagName=="A"){
					...
					var newEl = this.createElement(node.innerHTML);
					node.parentNode.replaceChild(newEl,node);
				}
			}else{
				//sel.focusNode
				var node = sel.focusNode;	
				if(sel.focusNode.tagName=="A"){
					//var newEl = this.createElement(node.innerHTML);
					node.parentNode.replaceChild(node.childNodes[0],node);
				}
			}
			that.deferFocus();
			sel.collapseToEnd();//光标移到最后
			that.syncValue();
		}
	},
