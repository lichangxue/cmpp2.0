...
Ext.de.PauseResumeUploadField = Ext.extend(Ext.form.Field, {
	initComponent: function () {
		Ext.de.PauseResumeUploadField.superclass.initComponent.call(this);
		this.autoCreate = {tag:"div",style:"",html:""};
	},
	onRender:function(ctnr,pos){
		if(!this.el){
			this.el = ctnr.createChild({tag: 'input', type: 'text',cls:'x-form-text x-form-field',value:'' });
			this.button = ctnr.createChild({tag: 'input', style:'margin-left:10px', type: 'button',value:'断点续传' });
		}
...
Ext.de.ImageUploader=Ext.extend(Ext.form.Field, {
	onRender: function (ctnr, pos) {
		this.el=ctnr.createChild({tag:"img",src:"../res/img/designTime/Ext.ux.ImageUploader.jpg"});
	}
});