...
Ext.listPosition=[["leftTop","左上"],["rightTop","右上"],["leftBtm","左下"],["rightBtm","右下"],["center","居中"],["tile","平铺"]];
 ...
 Ext.ex={}
    ...
    Ext.Toast.show = function(msgText, config) {  
        ...
        var win = Ext.MessageBox.show(msgCfg);  
        setTimeout(function() {  
			Ext.MessageBox.hide(); 
	...
	Ext.CMPP={
		alert:function(title,msg,config){
			...
			var win = Ext.Toast.show(msg,cfg);
			return win;			
		},
 ...
 Ext.ux.FormLayout = Ext.extend(Ext.layout.FormLayout, {
	...
	renderItem: function (c, position, target) {
		if (c && !c.rendered && c.inputType != 'hidden') {
            ...
            if(position){
                fld=tpl.insertBefore(position, args, true);
            }else{
                fld=tpl.append(target, args, true);
            }
			...
			//为了保证Note元素横向排列于控件右侧，需要针对combobox特殊处理 by cds
			if(c.id.toLowerCase().indexOf("combobox")!=-1){
				ctnrId=c.el.parent().id;
			}
	...
	return function(fld){
        ...
        if(stype=="task"){
            var iframe=new Ext.BoxComponent({
                autoEl:{tag:"iframe",name:iname,src:dir+"scriptdebug.jhtml?nodeId="+nodeId+"&id1="+id1+"&id2="+id2+"&stype="+stype+ (saveEnable==false?"&saveEnable="+saveEnable:"") + (tpl?"&tpl=" +　tpl:"")+"&scope=task"}
                ,anchor:"100%"
             });

        }else if(stype=="interf"){
             var iframe=new Ext.BoxComponent({
                  autoEl:{tag:"iframe",name:iname,src:dir+"scriptdebug.jhtml?nodeId="+nodeId+"&id1="+id1+"&id2="+id2+"&stype="+stype+ (saveEnable==false?"&saveEnable="+saveEnable:"") + (tpl?"&tpl=" +　tpl:"")+"&scope=interface"}
                  ,anchor:"100%"
             });

        }else if(stype=="dynPage"){
              var iframe=new Ext.BoxComponent({
                  autoEl:{tag:"iframe",name:iname,src:dir+"scriptdebug.jhtml?nodeId="+nodeId+"&id1="+id1+"&id2="+id2+"&stype="+stype+ (saveEnable==false?"&saveEnable="+saveEnable:"") + (tpl?"&tpl=" +　tpl:"")+"&scope=dyn"}
                  ,anchor:"100%"
              });

        }else if(stype=="dynPagePlus"){
              var iframe=new Ext.BoxComponent({
                  autoEl:{tag:"iframe",name:iname,src:dir+"scriptdebug.jhtml?nodeId="+nodeId+"&id1="+id1+"&id2="+id2+"&stype="+stype+ (saveEnable==false?"&saveEnable="+saveEnable:"") + (tpl?"&tpl=" +　tpl:"")+"&scope=dyn plus"}
                  ,anchor:"100%"
                  });
...
/*
*弹出帮助页面
*用法：Ext.CMPP.openHelper(2); 或 Ext.CMPP.openHelper("http://xxxxx")
*/
Ext.CMPP = Ext.CMPP || {};
Ext.CMPP.openHelper=function(helpData){
	var helpUrl ;
	if(!isNaN(helpData)){//ID
		helpUrl = "http://g.cmpp.ifeng.com/Cmpp/runtime/interface_343.jhtml?dataId=" +helpData　;
	}else{//url
		helpUrl = helpData;
	}
	...
	var iframe=new Ext.BoxComponent({
		autoEl:{tag:"iframe",name:"帮助文档",src:helpUrl},
		anchor:"100%"
	});
	win.add(iframe);
		
	win.show("",function(win){
		win.body.mask("正在加载中...");
