...
Designer = {
    ...
    customEditors: {//可共用的属性编辑器
		...
		HtmlViewer:{
			editor:new Ext.ux.CustomEditors.HtmlViewer({})
		},
		CustomEditors_TextField_alphanum: {
			editor:new Ext.grid.GridEditor(new Ext.form.TextField({vtype:'alphanum'}))
		},
        CustomEditors_TextArea: {
			editor:new Ext.ux.CustomEditors.TextArea({})
		},
        CustomEditors_TextArea_cb: {
			editor:new Ext.ux.CustomEditors.TextArea({ callback: "Designer.setJsFunction" })
		},
        JsonEditor:{
			editor:new Ext.ux.JsonEditor({})
		},
        ...
        ComboBox_1:{
			editor:(function(){
				var ed = new Ext.grid.GridEditor(new Ext.form.ComboBox({
					allowBlank:true,
					...
					getDisplayText:function(obj,value){
						var arr = obj.extra.loadStore;
		...
		ListColumn_1:{
			editor:new Ext.ux.ListItemEditor()
		},
		ArrayEditorField:{
			editor:new Ext.ux.CustomEditors.ArrayEditor()
		},
		DimenSingle:{
			editor:new Ext.ux.DimenSingleEditor()
		},
		DimenList:{
			editor:new Ext.ux.DimenListEditor()
		},
		Array:{
			editor:new Ext.ux.ArrayEditor()
		},		
		KeyValueEditor:{
			editor:new Ext.ux.CustomEditors.KeyValueEditor()
		},	
		Position:{
			editor:new Ext.ux.CheckGroupEditor({dataSource:Ext.listPosition})
		}
    },
    ...
    setJsFunction: function (value) {
        try {
            ...
            this.customEditors["Event"].editor.field.store.loadData(funNames);
            return true;
        } catch (ex) {
            Ext.Msg.show({
                title: '错误提示',
	...
	createCmp:function(ctrlList,ctnr,isEdit) {
		...
		while(++index<len){
			...
			//TODO:判断ID是否存在
			if(Ext.getCmp(id)){
				id=this.getId(uitype);
				alert("控件id:"+ ui["id"] +"已经存在!自动生成新的id:" + id);
			}
			ui["id"]=id;
	...
	/*插入控件到某个控件之后*/
	insertCmp:function(ctrlList,currentCtrl,isEdit,position){
		if(!currentCtrl.parent || !currentCtrl.ctrl) return;
		var parentCt=Ext.getCmp(currentCtrl.parent.id);
		var ctrl=Ext.getCmp(currentCtrl.ctrl.id);
		var positon=parentCt.items.indexOf(ctrl);
		if(positon==-1) positon=parentCt.items.length;
		
		if(!ctrlList||ctrlList.length==0||!currentCtrl.parent)
			return;
		var len=ctrlList.length;
		for(var i=len-1;i>=0;i--){
			var config=ctrlList[i];
			var controls=config.controls;
			delete config.controls;//preProcCfg shouldn't run repeatedly.
			(isEdit&&this.preProcCfgForEdit(config));
			...
			uiobj.formField.uitype = uitype;
			Designer.objFunction.createProps.call(uiobj.formField,uiobj.id,config,position);
			uiobj.formField.on("click", Designer.objFunction.objClick);		
    ...
    */
    ...
    insertCtrl: function (propsObj, ctnrId,position) {
        if (!ctnrId) {//must be form;
			...
			if(typeof position=="number"){
				ctnr.controls.splice(position,0,propsObj);
			}else{
    ...
    formatFormCfg: function () {
		...
		if(!formConfig.form.listPage){
			formConfig.form.listPage={
				"l_listCfg" : "[]",
				"l_template" : "listTemplate_default_1",
				"l_customedTpl" : "",
				"l_pagesize" : 20
			}
		}
...
Designer.objFunction={
	...
	createProps: function (ctrlId,formCfg,position) {
		...
		if (cfg.design.ui.isContainer)
            propObj.controls = [];
		Designer.insertCtrl(propObj, this.ctnrId,position);
		if (uitype != "FormPanel") {
			var toolbar = Ext.DomHelper.append(this, { 
				cls: "toolbar",
				html:'<button class="btnCopy">复制</button><button class="btnPaste">粘贴到</button><button class="btnDelete">删除</button>'
			}, true);
			this.toolbar = toolbar;
			var ths = this;
			toolbar.child('.btnDelete').on("click", function () {
			...
			toolbar.child('.btnCopy').on('mousedown',function(event,dd){
				event.stopPropagation();
				event.preventDefault();

				var obj = Ext.fly(event.target);
				var ctrlId=this.dom.ctrl.id;
				var ctrlObj=Designer.searchCtrl(ctrlId);
				if(!ctrlObj || !ctrlObj.ctrl) return;
				
				window._clipDataForCopy = Ext.encode(ctrlObj.ctrl);
				window.clipSwfDiv.position("absolute",999999999,obj.getLeft(),obj.getTop());
				window.clipSwfDiv.setSize(obj.getWidth(),obj.getHeight());
				window.clipSwfDiv.first().setSize(obj.getWidth(),obj.getHeight());
				
				window.clipSwfDiv.on('mouseup',function(){
					window.clipSwfDiv.position("absolute",1,-100,-100);
					Ext.CMPP.alert('提示','已复制');		
				},this,{single:true});
				
			},this);
			
			toolbar.child('.btnPaste').on("click", function (e,obj) {
				
				var win = new Ext.Window({
					title:"请按Ctrl+V粘贴复制的代码到数据框里",
					width:500, 
					height:400,
					buttonAlign: "center",
					closable:true ,
					...
					items:[{
						xtype:"textarea",
						emptyText:"请按Ctrl+V粘贴复制的代码到文本框中"
					}],
					buttons:[{
						"text":"确定",
						scope:this,
						handler:function(that){
							var currentCtrlId=this.dom.ctrl.id;
							var clipData = that.ownerCt.items.get(0).getValue();
							try{
								var ctrlConfig = Ext.decode(clipData);
								var ctrlUi= Designer.getControl(ctrlConfig);
								var currentCtrl = Designer.searchCtrl(currentCtrlId);	
								var position = Designer.getCtrlIndex(currentCtrl.ctrl.id,currentCtrl.parent.controls);
								
								Designer.insertCmp([ctrlUi],currentCtrl,true,position);
								
								//Designer.insertCtrl(ctrlConfig, currentCtrl.parent.id,position);
							}catch(ex){
								Ext.CMPP.alert('提示','数据格式不合法');
								return;
							}
							that.ownerCt.close();
						}
